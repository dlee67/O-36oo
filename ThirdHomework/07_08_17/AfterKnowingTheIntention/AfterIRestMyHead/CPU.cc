...
void send_signals (int signal, int pid, int interval, int number)
{
    ...
    {
		//This interval is the place where the activity of the process can happen.
        sleep (interval);
...
void ISR (int signum)
{
	...
	//The line below is what's calling the scheduler.    
	ISV[signum](signum);
}
...
void boot (int pid)
{
    ...
    if ((ret = fork ()) == 0)
    {
        ...
        send_signals (SIGALRM, pid, 2, NUM_SECONDS);
...
void create_idle ()
{
    ...
    if ((idlepid = fork ()) == 0)
    {
        ...
        {
			sleep(1);
			cout << "Spinning with sleep(1)" << endl;
